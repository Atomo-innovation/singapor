...
def run_webcam():
    ...
    cap = cv2.VideoCapture(0)       # Webcam index (0 = default cam)
    if not cap.isOpened():
    ...
    while True:
        ...
        for idx, (face, ext) in enumerate(zip(faces, extends)):
            ...
            # Attribute text
            info = [
                f"{gender_tags[ext.gender]}",
                f"{race_tags[ext.race]}",
                f"{age_bracket_tags[ext.age_bracket]}",
                f"{emotion_tags[ext.emotion]}"
            ]
            y0 = y1 - 10
            for i, txt in enumerate(info):
                cv2.putText(draw, txt, (x1, y0 - i * 18),
                            cv2.FONT_HERSHEY_SIMPLEX, font_scale,
                            (0, 0, 0), line_thickness)

        cv2.imshow("InspireFace Webcam", draw)
